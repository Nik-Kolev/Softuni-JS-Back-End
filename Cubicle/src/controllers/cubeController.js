...
cubeController.get('/:id/attach-accessories', isAuth, async (req, res) => {
...
cubeController.post('/:id/attach-accessories', isAuth, async (req, res) => {
...
cubeController.get('/:id/delete', isAuth, async (req, res) => {
    const cube = await cubeService.getSingleCubeById(req.params.id).lean();
    const options = selectLevel(cube.difficultyLevel);
    res.render('cube/delete', { cube, options });
});

cubeController.post('/:id/delete', isAuth, async (req, res) => {
    await cubeService.delete(req.params.id);
    res.redirect('/');
});

cubeController.get('/:id/edit', isAuth, async (req, res) => {
    const cube = await cubeService.getSingleCubeById(req.params.id).lean();
...
cubeController.post('/:id/edit', isAuth, async (req, res) => {
